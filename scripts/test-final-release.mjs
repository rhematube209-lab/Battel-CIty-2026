...
assert(buildInfoSrc.includes("version: '1.0.0'"), "BUILD_INFO defines version '1.0.0'");
assert(!buildInfoSrc.includes("channel: 'RC'"), 'BUILD_INFO does NOT retain "RC" channel');

const packageJson = JSON.parse(readFileSync('package.json', 'utf-8'));
assert(packageJson.version === '1.0.0', `package.json version is '1.0.0' (got '${packageJson.version}')`);
assert(packageJson.version === '1.0.0', 'BUILD_INFO.version matches package.json version exactly');
