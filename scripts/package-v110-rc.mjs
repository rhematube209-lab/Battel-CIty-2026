...
const ZIP_NAME = 'battle-city-2026-v1.1.0-rc.1.zip';
const ZIP_PATH = path.resolve(ROOT_DIR, ZIP_NAME);
const SUMS_FILE = path.resolve(ROOT_DIR, 'SHA256SUMS_v1.1.0-rc.1.txt');
const TEST_EXTRACT_DIR = path.resolve(ROOT_DIR, 'scratch/test-rc-extract');

console.log('=============================================================');
console.log('PHASE 24 — RELEASE CANDIDATE PACKAGING & INTEGRITY VERIFICATION');
console.log('=============================================================\n');
...
// 4. Generate SHA256SUMS_v1.1.0-rc.1.txt
...
console.log('\nVerifying static HTTP serving of extracted archive on port 3009...');
const server = http.createServer((req, res) => {
  const parsed = new URL(req.url || '/', 'http://127.0.0.1:3009');
...
await new Promise((resolve) => server.listen(3009, '127.0.0.1', resolve));

const testReq = await fetch('http://127.0.0.1:3009/');
if (testReq.status !== 200) {
  server.close();
  throw new Error(`HTTP fetch failed with status ${testReq.status}`);
}
const htmlText = await testReq.text();
if (!htmlText.includes('Battle City 2026') && !htmlText.includes('battlefieldCanvas')) {
  server.close();
...
console.log('\n=============================================================');
console.log('RELEASE CANDIDATE PACKAGING VERIFICATION: ALL PASSED');
console.log('=============================================================\n');
